-- [回答]/* ■ [回答]と記載のある箇所へ、1〜8の各課題内容に沿ったSQL文を記述しなさい。 */

-- 1. 店舗テーブルから店舗名を抽出しなさい。また、列名の表示は別名で'店舗名'とすること。
-- [回答]SELECT  store_name AS store_nameAbc FROM store_table;
...
-- [回答]SELECT store_table.store_name, goods_table.goods_name, stock_table.quantity
FROM stock_table
INNER JOIN store_table ON stock_table.quantity::varchar = store_table.store_name
INNER JOIN goods_table ON stock_table.quantity::varchar= goods_table.goods_name;
...
-- [回答]INSERT INTO goods_table(goods_code,goods_name,price,update_day)
...
SET 
    "quantity" =10,"update_day" = CURRENT_DATE
